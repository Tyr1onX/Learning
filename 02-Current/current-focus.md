...
- 每天最多安排一个旧基础知识主题，避免和新主题、算法任务叠加过重；
- 多个节点同时到期时，优先最早逾期项，其余顺延；
- 必须通过用户实际回答判定通过 / 易错 / 待复习，不能自动总结后直接标记通过；
- 晚间复习同样必须先提问、等待回答，再回写真实结果；
- 每个知识主题至少保留 D+7 与 D+21 两个长期验证节点；若某次未通过，则按实际情况追加更近的复习节点。

当前待验证队列：

- 变量 / 基础类型：D+7 原定 2026-09-03，当前逾期；D+21：2026-09-17；
- 对象 / 数组：D+7 原定 2026-09-04，当前逾期；D+21：2026-09-18；
- 函数定义与调用：D+7 原定 2026-09-05，当前逾期；D+21：2026-09-19；
- 词法作用域 / 闭包：D+7：2026-09-06；D+21：2026-09-20；
